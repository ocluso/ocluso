...
const generatedWarning = "/* This file was generated by gen-moduleindex.go - please do not edit */"
...
func buildGoModuleIndex(moduleNames []string) {
	f, err := os.Create("pkg/moduleindex/moduleindex.go")
	handleError(err)

	defer f.Close()

	fmt.Fprintln(f, generatedWarning)
...
func buildJavaScriptModuleIndex(moduleNames []string) {
	f, err := os.Create("frontend/moduleindex/moduleindex.js")
	handleError(err)
	...
	fmt.Fprintln(f, "\nexport default {")
	for _, name := range moduleNames {
		fmt.Fprintln(f, "  '"+name+"': import('../../modules/"+name+"/frontend/module.mjs'),")
	}
	fmt.Fprintln(f, "}")
}
...
func main() {
	...
	buildGoModuleIndex(moduleNames)
	buildJavaScriptModuleIndex(moduleNames)
}
